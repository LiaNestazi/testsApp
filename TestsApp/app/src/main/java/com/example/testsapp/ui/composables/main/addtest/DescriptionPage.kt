...
@Composable
fun DescriptionPage(navController: NavHostController){
    var desc = remember{
        mutableStateOf(SingletoneTypes.instance.created_test.description)
    }
    var label = remember {
        mutableStateOf("Введите описание теста")
    }
    ...
    Box(modifier = Modifier.fillMaxSize()){
        ...
        Button(onClick = {
            if (desc.value != ""){
                SingletoneTypes.instance.created_test.description = desc.value
                navController.navigate("TypeTestPage")
            } else {
                isError.value = true
            } },
